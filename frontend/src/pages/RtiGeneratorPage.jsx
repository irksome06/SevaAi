...
import { ArrowLeft, Copy, Download, FileText, Pencil, Printer } from 'lucide-react';
...
(Signature of Applicant)`, [draft]);
  ...
  return <div className="rti-page">
    ...
    <main className="rti-main">
      ...
      <section className="rti-actions print-hide"><div><button type="button" onClick={() => setEditing((current) => !current)}><Pencil size={16} /> {editing ? 'Close editor' : 'Edit details'}</button><button type="button" onClick={copy}><Copy size={16} /> Copy text</button><button type="button" onClick={download}><Download size={16} /> Download</button></div><button type="button" className="rti-primary" onClick={() => window.print()}><Printer size={16} /> Generate PDF</button></section>
    </main>
