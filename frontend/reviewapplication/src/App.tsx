...
(Signature of Applicant)`,
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] text-[#191c1e]">
      <header className="sticky top-0 z-20 border-b border-[#c9ccd5] bg-[#f7f8fa]/95 backdrop-blur print:hidden">
        <div className="mx-auto flex h-16 max-w-[1100px] items-center justify-between px-4 md:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[16px] font-bold leading-5 text-[#003fb1]">RTI Mitra</h1>
              <span className="hidden text-xs text-[#626571] md:block">Application assistant</span>
            </div>
          </div>
          <div className="hidden items-center gap-3 md:flex">
            ...
            <span className="text-sm text-[#626571]">Preview document</span>
          </div>
      ...
      <main className="mx-auto flex w-full max-w-[1100px] flex-col items-center gap-8 px-4 py-8 pb-32 md:px-8 md:py-12 md:pb-10">
        <section className="max-w-2xl text-center">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-[#003fb1]">Final review</p>
        ...
        {isEditing && (
          <section className="w-full max-w-[800px] rounded border border-[#c9ccd5] bg-white p-5 shadow-sm print:hidden md:p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {([
                ['fullName', 'Full name'], ['fatherSpouseName', "Father or spouse name"], ['address', 'Permanent address'], ['phone', 'Contact number'], ['email', 'Email address'], ['department', 'Department / authority'], ['departmentAddress', 'Department address'], ['locality', 'Locality'], ['ward', 'Ward number'], ['financialYear', 'Financial year'], ['street', 'Street name'], ['place', 'Place'], ['date', 'Date'],
              ] as [keyof Draft, string][]).map(([field, label]) => (
                <label className="flex flex-col gap-1.5 text-sm font-semibold text-[#434654]" key={field}>
                  {label}
                  <input className="rounded border border-[#aeb2bf] bg-[#fbfcfd] px-3 py-2.5 font-normal text-[#191c1e] outline-none transition focus:border-[#003fb1] focus:ring-2 focus:ring-[#dbe1ff]" value={draft[field]} onChange={(event) => updateDraft(field, event.target.value)} />
                </label>
              ))}
            </div>
